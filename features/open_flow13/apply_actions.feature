...
Feature: Apply
  ...
  Scenario: new(SendOutPort.new(1))
    When I try to create an OpenFlow instruction with:
      """
      Pio::OpenFlow::Apply.new(Pio::OpenFlow::SendOutPort.new(1))
      """
    Then it should finish successfully
    And the message has the following fields and values:
      | field               |                        value |
  ...
  Scenario: read
    When I try to parse a file named "open_flow13/apply_actions.raw" with "Pio::OpenFlow::Apply" class
    Then it should finish successfully
    And the message has the following fields and values:
      | field               |                        value |
      | class               |       Pio::OpenFlow13::Apply |
